...
pub fn solve(input: &str) -> u64 {
    ...
    let height = map.iter().count() as i32;
    let width = map[0].iter().count() as i32;
    ...
    nodes_per_char.iter().for_each(|(char, points)| {
        ...
        pairs.iter().for_each(|(left, right)| {
            ...
            antinodes.insert(left.clone());
            antinodes.insert(right.clone());
            let mut finger = AntiNode {
                x: left.x + diff_x,
                y: left.y + diff_y,
            };
            while (finger.x >= 0 && finger.x < width && finger.y >= 0 && finger.y < height) {
                antinodes.insert(finger.clone());
                finger = AntiNode {
                    x: finger.x + diff_x,
                    y: finger.y + diff_y,
                }
